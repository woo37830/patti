$(document).ready(function () {
  ...
  $(document).on("click", "li", function (evt) {
    ...
    try {
      window.location.href =
        "/sc_demo/dispatcher?event=_start&process=" +
        processName +
        "&app=" +
        parentName +
        "&userToken=" +
      ...
      var errMsg =
        "<doc><errors><error>" + err + "</error></errors><tasks></tasks></doc>";
      window.location.href =
        "/sc_demo/home.jsp?process=tasklist&app=Home&error=" +
        errMsg +
        "&userToken=" +
        theToken;
      evt.stopPropagation();
  ...
  $(document).on("click", "#home-td", function () {
    var theToken = $("#token").text();
    window.location.href =
      "/sc_demo/home.jsp?process=tasklist&app=Home&userToken=" + theToken;
  });
  ...
  var auto_refresh = setInterval(function () {
    ...
    $("#error-div").text("");
    window.location.href =
      "/sc_demo/home.jsp?app=Home&process=tasklist&userToken=" + theToken;
    //  $(document).find("#tasklist").dataTable().columnFilter();
  }, 15000); // autorefresh the content of the div after every 30000 milliseconds(30sec)
  ...
  $(document)
    ...
    .submit(function (event) {
      ...
      return true;
      //var request = $.ajax({
      //    url: "http://localhost:8080/sc_demo/dispatcher",
      //    type: "post",
      //    data: serializedData})
      //    .done(function(){
      //    })
...
function showBugList() {
  window.open("http://jwooten37830.com/mantisbt-1.2.10", "_blank");
}

function showNoteList() {
  window.open("http://jwooten37830.com/blog", "_blank");
}
...
function toggleChecks(theForm, menuObj) {
  ...
  if (text.length > 0) {
    if (str.indexOf("|" + text + "|") != -1) removeSelectedItem(text);
    else addSelectedItem(text);
  }
  if (str.length != 0) {
    setDirtyFlag(true);
