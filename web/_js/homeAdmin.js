$(document).ready(function () {
  ...
  if ($_errorDiv.text() != null) {
    // copy the one coming back from the service to the location
  ...
  $(document).on("click", "#tasklist tr", function () {
    ...
    if (oid != null && oid.length > 0) {
      ...
      window.location.href =
        "/sc_demo/dispatcher?event=_inspect&oid=" +
        oid +
        "&userToken=" +
        theToken;
    } else {
      alert(
        "This entry contains incorrect information and cannot be de-queued!\n" +
          "oid = " +
          oid
      );
    }
  ...
  $(document).on("click", "#selectId", function () {
    ...
    window.location.href =
      "/sc_demo/queueAdmin.jsp?app=Home&select=" +
      theStatus +
      "&userToken=" +
      theToken;
  });
